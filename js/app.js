(() => {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".banner-section__slider")) new core(".banner-section__slider", {
            ...
            if (document.querySelector(".video-overviews__slider")) new core(".video-overviews__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 2,
                ...
                lazy: true,
                navigation: {
                    prevEl: ".video-overviews__btn_prev",
                    nextEl: ".video-overviews__btn_next"
                },
                on: {}
            });
            if (document.querySelector(".gallery__slider")) {
                ...
                function gallerySliderFix() {
                    const galleryContainer = document.querySelector(".gallery__block");
                    const diff = (window.innerWidth - galleryContainer.offsetWidth) / 2;
                    if (diff > 0) document.querySelector(".gallery__wrapper").style.width = document.querySelector(".gallery__slider").offsetWidth + diff + 15 + "px"; else document.querySelector(".gallery__wrapper").style.width = document.querySelector(".gallery__slider").offsetWidth + 15 + "px";
                }
        ...
        da.init();
        window.addEventListener("load", (function(e) {
            const alignBlocks = function(item) {
                let height = 0;
                const items = document.querySelectorAll(item);
                items?.forEach((elem => {
                    if (!elem.scrollHeight || 0 === elem.scrollHeight) return;
                    if (elem.scrollHeight > height) height = elem.scrollHeight;
                }));
                items?.forEach((elem => {
                    elem.style.height = `${height}px`;
                }));
            };
            alignBlocks(".item-prodct__content");
            const videosContainerFull = document.querySelector(".video-overviews__wrapper");
            let videoPlay = false;
            videosContainerFull?.addEventListener("click", (function(e) {
                let clickedButton = e.target.closest(".video-paused__btn");
                let videoBlockParrent = e.target.closest(".video-block");
                let video = videoBlockParrent.querySelector("video");
                let closestButton = videoBlockParrent.querySelector("button");
                const allVideoButtons = document.querySelectorAll(".video-paused__btn");
                if (!videoPlay) {
                    clickedButton.classList.add("video-play_btn");
                    clickedButton.classList.remove("video-paused__btn");
                    clickedButton.classList.remove("_icon-play");
                    videoBlockParrent.classList.remove("open");
                    videoBlockParrent.classList.add("closed");
                    video.play();
                    videoPlay = true;
                    allVideoButtons.forEach((videoBtn => {
                        videoBtn.setAttribute("disabled", true);
                    }));
                    clickedButton.removeAttribute("disabled");
                } else if (videoPlay) {
                    closestButton.classList.remove("video-play_btn");
                    videoBlockParrent.classList.remove("closed");
                    videoBlockParrent.classList.add("open");
                    closestButton.classList.add("video-paused__btn");
                    closestButton.classList.add("_icon-play");
                    video.pause();
                    videoPlay = false;
                    allVideoButtons.forEach((videoBtn => {
